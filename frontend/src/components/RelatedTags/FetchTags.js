...
const TaggedArticles = () => {
  const { tag } = useParams(); // useParams hook'u ile URL'deki tag parametresini alıyoruz
  const decodedTag = decodeURIComponent(tag); // Etiketi decode ediyoruz
  ...
  return (
    <div className="flex flex-col md:justify-center md:items-center bg-gray-50 dark:bg-black w-full">
      <div className="flex flex-col md:max-w-[1200px] md:w-full gap-5">
        <h1 className="text-3xl text-gray-900 dark:text-white p-5">Etikete Göre Makaleler: {decodedTag}</h1>
        {displayedArticles.map((article, index) => (
          <div key={article.id || index} className="flex flex-col gap-10 border-t p-5">
